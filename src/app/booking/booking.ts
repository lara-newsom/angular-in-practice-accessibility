  ...
  bookingForm = form(this.bookingDetails, (schemaPath) => {
    ...
    validate(schemaPath.checkInDate, ({ value }) => {
      ...
      return checkInDate >= today
        ? null
        : { kind: 'checkInDateHasPassed', message: 'Check-in date cannot be in the past' };
    });
    validate(schemaPath.checkOutDate, ({ value, valueOf }) => {
      ...
      return checkOutDate >= checkInDate
        ? null
        : {
            kind: 'checkOutDateIncorrect',
            message: 'Check-out date cannot be before or the same as check-in date',
          };
    });
  });
